...
unsigned int FileSystem::parseFilesystem(PartitionEntry *partition,
  ...
  while (i < blockSize - 1) {
    ...
    if (passNumber == 1 && performCheck == 1) {
      if (fileEntry->inode != currentInode
        ...
        cout << "Entry '.' has inode " << fileEntry->inode << " instead of "
             << currentInode << ". This will be fixed." << endl;
        fileEntry->inode = currentInode;
        writeSectors(blockSector, (blockSize / SECTOR_SIZE_BYTES), buf);
      }
      if (fileEntry->inode != parentInode
          && (!strcmp(fileEntry->name, ".."))) {
        cout << "Entry '..' has inode " << fileEntry->inode << " instead of "
             << parentInode << ". This will be fixed." << endl;
  ...
  if (fileEntry != NULL && fileEntry->rec_len > 8 + fileEntry->name_len
      && (fileEntry->rec_len - 8 - fileEntry->name_len) > 16
      && !performCheck)
    return blockNumber;
  return -1;
}
...
unsigned int FileSystem::readIndirectDataBlocks(PartitionEntry *partition,
                                                ...
                                                int passNumber, int performCheck,
                                                int fileType) {
  unsigned int i = 0;
  int retValue = -1;
  unsigned char buf[blockSize];
  ...
  for (i = 0; i < blockSize; i += 4) {
    ...
    if (block != 0 && (indirectionLevel == 3 || indirectionLevel == 2)) {
      ...
      if (fileType != 1)
        retValue = parseFilesystem(partition, block, passNumber, inode, parentInode,
                                  performCheck);
      if (passNumber == 4) {
        blockMap[block] = 1;
        continue;
...
unsigned int FileSystem::readDataBlocks(PartitionEntry *partition,
  ...
  if (pointers[12] != 0)
    retValue = readIndirectDataBlocks(partition, inode, parentInode, pointers[12], 1,
                                     passNumber, performCheck, fileType);
  if (!performCheck && retValue != -1)
    return retValue;

  if (pointers[13] != 0)
    retValue = readIndirectDataBlocks(partition, inode, parentInode, pointers[13], 2,
                                     passNumber, performCheck, fileType);
  if (!performCheck && retValue != -1)
    return retValue;

  if (pointers[14] != 0)
    retValue = readIndirectDataBlocks(partition, inode, parentInode, pointers[14], 3,
                                     passNumber, performCheck, fileType);
  return retValue;
}
...
void FileSystem::setBlockBitmap(PartitionEntry *partition,
  ...
  writeSectors(blockBitmapSector, (blockSize / SECTOR_SIZE_BYTES),
               blockBitmap);
}
...
void FileSystem::readRootInode(PartitionEntry *partition) {
  ...
  for (i = 11; i <= super_block.s_inodes_count; i++) {
    ...
    if (bitmapValue == 1 && inodeMap[i] == 0) {
      ...
      if (in.fileType != 0) {
        cout << "Inode " << i
             << " has invalid entry in inode bitmap. Bitmap value:"
             << bitmapValue << ", collected value:" << inodeMap[i]
             << ". This will be fixed.\n" << endl;
    ...
    if (inodeLinkCount[i] != in.hardLinksQt) {
      if (in.fileType != 0) {
        cout << "Inode " << i
             ...
             << in.hardLinksQt << ", collected value: " << inodeLinkCount[i]
             << ". This will be fixed" << endl;
      }
  ...
  for (i = 1; i <= super_block.s_blocks_count; i++) {
    ...
    if (i < firstRootBataBlock && checkBlockBitmap(partition, i) == 0) {
      cout
          << "Block "
          << i
          << " has invalid entry in block bitmap. Bitmap value: 0, collected value: 1. This will be fixed."
          << endl;
      setBlockBitmap(partition, i, 1);
    }
    if (blockMap[i] == 1 && bitmapValue != blockMap[i]) {
      cout << "Block " << i
           << " has invalid entry in block bitmap. Bitmap value: "
           << bitmapValue << ", collected value: " << blockMap[i]
           << ". This will be fixed." << endl;
      setBlockBitmap(partition, i, 1);
    }
  }
